...
level3의 문제 소스의 경우 버퍼 오버플로우 취약점이 존재한다. 먼저 대략적인 개념을 알아본다.

먼저 위에서 어셈블리어를 가져와 확인해보자아~
...
함수의 프롤로그 부분이다. `sub 0x78,esp`를 통해 120바이트 만큼의 스택을 확보하는 것을 확인할 수 있다. 그렇다면 `0x78`+`0x4(SFP)`+`0x4(RET)`를 계산하면 124+쉘 코드 실행 주소 형태로, `main`함수가 끝나고 리턴 되는 시점에 쉘이 실행된다.

| cmd[100] |  SFP   |  RET   |
| :------: | :----: | :----: |
| 120 byte | 4 byte | 4 byte |

| Str[5] | NOP[90]  | Shellcode[25] | SFP[4]  | RET[4]       |
| ------ | -------- | ------------- | ------- | ------------ |
| dig @  | 'A' * 90 | 쉘 코드       | 'A' * 4 | 쉘 코드 주소 |

책에는 이런식으로 표가 나와있다... 그런데 음.... 좋긴한데 갑작스럽다 라는 생각이 든다. 맛보기라 그런가보다.
